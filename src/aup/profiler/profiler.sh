...
if [[ -z $TEXTFILE ]] && [[ -z $MODELS ]]; then
    ...
    if test -f "$OUTPUTFILE"; then
        python -m aup.profiler.calculate $OUTPUTFILE
    fi
    exit 1
fi
...
if [[ -n $TEXTFILE ]]; then
    ...
    python -m aup.profiler.compile_stats $ENVIRONMENT $TEXTFILE
...
if [[ -n $MODELS ]]; then 
    ...
    python -m aup.profiler.compile_stats $ENVIRONMENT $MODELS
fi
